...
def update_ip_pool_logic(spider_name, link, logger):
    """
        ...
        补充：检查ip池+高概率ip集合，然后清除二者不可用的ip保留可用的ip，每次获取的ip不检查，直接入redis
    """
    # 连接redis
    ...
    while True:
        try:
            ...
            url = "http://ttvp.daxiangip.com/ip/?tid=559436287116377&delay=1&category=2&foreign=none&ports=8080,3128,80,808,1080,8081,8998,8123,8888,9999,8118,9797,3129,87,8083,8090,8799,8000,9000,81,8088&num=40"
            ...
            if len(ips) >= 40:
                ...
                m_ips = set( list(ip_pool) + list(high_prob_ips) )
                ...
                for id, ip_port in enumerate(m_ips):
                    s = "curl --connect-timeout 2 -m 2 -o /dev/null -s -w %{http_code} -x " + ip_port + " " + link
                    print s
                ...
                for ip in ips:
                    # 存redis
                    ua = random.choice(ua_pool) or ""
                    redis_srv.sadd(ip_pool_key, "%s||%s" % (ip, ua))
...
def main():
    ...
    update_ip_pool_simple(spider_name, link, logger)
